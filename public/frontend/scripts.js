function downloadFile(url, dataset, filename) {
    $.ajax({
        url: url,
        cache: false,
        data: dataset,
...
;$(document).ready(function (){
    ...
    $(document).on('click', '#actor-export-pdf', function (){
        let dataset = {
        ...
        let url = '/export/download_pdf';
        let filename = dataset.day !== "" ? `export_${dataset.day}.pdf` : `export_full.pdf`;

        downloadFile(url, dataset, filename);
    });

    $(document).on('click', '#actor-export-xls', function (){
        let dataset = {
            'city': $(`input[name="hidden_search_city"]`).val(),
            'district': $(`input[name="hidden_search_district"]`).val(),
            'street': $(`input[name="hidden_search_street"]`).val(),
        ...
        let url = '/export/download_xls';
        let filename = dataset.day !== "" ? `export_${dataset.day}.xlsx` : `export_full.xlsx`;
